...
<!-- ### 3.2 Padronização (ESLint, Prettier, EditorConfig)

Antes de escrevermos muito código, precisamos garantir que ele fique padronizado e legível, independentemente de quem está editando. Por isso, vamos configurar três ferramentas:

1. **ESLint** → analisa o código, avisa sobre erros e más práticas e ajuda a seguir boas regras no React.

2. **Prettier** → formata o código automaticamente (indentação, aspas, vírgulas etc.), mantendo tudo organizado.

3. **EditorConfig** → define padrões para o editor (indentação, charset, quebra de linha), garantindo consistência entre diferentes máquinas.

Instale dependências e arquivos básicos:
...
Atualize `package.json` (scripts úteis):
